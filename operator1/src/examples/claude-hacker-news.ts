...
import { createClaudeBrowserEngine } from "../engines/claude-browser-engine.js";
import type {
  BrowserRuntime,
...
function resolveBrowserExecutablePath(): string | undefined {
  if (
    typeof process.env.OPERATOR1_BROWSER_PATH === "string" &&
    process.env.OPERATOR1_BROWSER_PATH.length > 0
  ) {
    return process.env.OPERATOR1_BROWSER_PATH;
  }
